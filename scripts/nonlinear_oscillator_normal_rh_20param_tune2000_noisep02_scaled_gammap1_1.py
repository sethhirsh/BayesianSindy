...
def nonlinear_oscillator_sunode_library(t, y, p):
    ...
    #res = p.pn @ state 
    du_dt =p.pn[0] * 0.1 * y.u**3 + p.pn[1] * y.v**3 + p.pn[4] * y.u + p.pn[6] * y.v + p.pn[8] * y.u**2 + p.pn[10] * y.v**2 + p.pn[12] * y.u*y.v + p.pn[14] * y.u**2 * y.v + p.pn[16] * y.v**2 * y.u + p.pn[18]
    dv_dt = p.pn[2] * y.u**3 + p.pn[3] * 0.1 * y.v**3 + p.pn[5] * y.u + p.pn[7] * y.v + p.pn[9] * y.u**2 + p.pn[11] * y.v**2 + p.pn[13] * y.u*y.v + p.pn[15] * y.u**2 * y.v + p.pn[17] * y.v**2 * y.u + p.pn[19]
...
with model_sunode:
    ...
    pm.backends.save_trace(trace,+ 'nonlinear_oscillator_normal_rh_20param_tune2000_noisep02_scaled_gammap1_1.trace',model_sunode)
