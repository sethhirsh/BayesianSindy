...
with model_rh:
    ...
    trace_rh = pm.backends.load_trace('../nonlinear_oscillator_normal_rh_20param_tune2000_noisep02_unscaled_gammap1_2.trace',model_rh)
...
with model_sunode:
    ...
    start['y0'] = trace_rh['y0'].mean(axis=0)
    print(start)

    trace = pm.sample(2000, tune=2000, cores=2,random_seed=3,target_accept=0.9,start=start)

    #trace = pm.sample(2000, tune=2000, cores=2, random_seed=1, step_kwargs={'nuts':{'target_accept':0.9}})

    pm.backends.save_trace(trace,str(__file__)[:-3] + '.trace',model_sunode)
